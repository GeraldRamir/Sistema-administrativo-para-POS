...
export default async function DocumentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <h3 className="text-sm font-semibold text-foreground">Nuevo documento o borrador</h3>
        {clients.length === 0 ? (
      ...
      <div>
        <h3 className="text-sm font-medium text-foreground">Listado (últimos 500)</h3>
        {docs.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">Aún no hay filas. Use el formulario superior o la ficha de un cliente.</p>
        ) : (
              ...
              <tbody className="divide-y divide-border">
                {docs.map((d) => (
                  <tr key={d.id} className="bg-card">
                    <td className="px-3 py-2 text-muted-foreground">{d.createdAt.toLocaleString("es-DO")}</td>
                    <td className="px-3 py-2">{documentKindLabel[d.kind]}</td>
                    <td className="px-3 py-2 font-medium text-foreground">{d.title}</td>
                    <td className="px-3 py-2">
                      <Link className={link} href={`/clients/${d.client.id}`}>
